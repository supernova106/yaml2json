...
class Queries(object):
    ...
    def __init__(self, queries, query_attributes, query_defaults,
            outputWriters, monitor_host, monitor_port, output_type, monitor_database=None, monitor_username=None, monitor_password=None):
        """
        ...
        self.monitor_database = monitor_database
    ...
    def create_host_entry(self, host_name, query_names, query_port, username, password, urlTemplate, aliasTemplate, set_name, rootPrefix):
        ...
        (hostShortName, sep, rest) = str(host_name).partition(".")
    ...
    def create_host_set_configuration(self, host_names, query_names, query_port, username, password, urlTemplate, set_name, global_host_alias):
        ...
        for host_name in host_names:
            ...
            (host, aliasSep, alias) = str(host_name).partition(";")
            if aliasSep == "":
            ...
            (alias, aliasSep, rootPrefix) = str(alias).partition("/")
            if aliasSep == "":
                rootPrefix = "servers"

            (host, sep, port) = str(host).partition(":")
            if sep == "":
                port = query_port
            host = host.strip()
            alias = alias.strip()
    ...
    def create_output_writer_configuration(self, typeName, rootPrefix):
        ...
        if len(self.outputWriters) == 0 and self.output_type == 'graphite':
            return [ {
        ...
        if len(self.outputWriters) == 0 and self.output_type == 'influxdb':
            return [ {
...
class HostSets(object):
    ...
    def __init__(self, host_sets):
        ...
        for host_set in host_sets:
            ...
            # Default is graphite
            if 'output_type' in host_set:
                set_entry['output_type'] = host_set['output_type']
            else:
                set_entry['output_type'] = 'graphite'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='jmxtrans output writer')
    parser.add_argument('--file', metavar='FILE', type=str, help='INPUT.yaml', required=True)
    args = parser.parse_args()
    file_config = args.file

    infile = open(file_config, 'r')
    yf = yaml.load(infile)
    ...
    influxdb_host = yf['influxdb_host'] if 'influxdb_host' in yf else None
    influxdb_port = yf['influxdb_port'] if 'influxdb_port' in yf else None
    influxdb_database = yf['influxdb_database'] if 'influxdb_database' in yf else None
    influxdb_username = yf['influxdb_username'] if 'influxdb_username' in yf else None
    influxdb_password = yf['influxdb_password'] if 'influxdb_password' in yf else None
    h = HostSets(yf['sets'])

    for set_name in h.set_names():
        outfile = open(set_name + ".json", 'w')
        s = h.get_set(set_name)
        if s['output_type'] == 'graphite':
            query_attributes = [
            ...
            outputWriters, graphite_host, graphite_port, s['output_type'])
        elif s['output_type'] == 'influxdb':
            query_attributes = [
                "obj", "resultAlias", "attr", "typeName",
            ] 
            q = Queries(yf['queries'], query_attributes, yf.get('query_defaults', {}),
            outputWriters, influxdb_host, influxdb_port, s['output_type'], 
            monitor_database=influxdb_database, monitor_username=influxdb_username, monitor_password=influxdb_password)
